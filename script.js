document.addEventListener("DOMContentLoaded", () => {
  let balance = 10000;
  let coins = [
    { name: "$HCKTRD", price: 0.05, lastPrice: 0.05, trend: 0.01, volume: 1000000, marketCap: 1000000000 }
  ];
  let positions = [];
  let tradeHistory = [];
  ...
  function updateMarket() {
    coins.forEach(coin => {
      ...
      coin.marketCap = coin.price * coin.volume; // Update market cap to reflect the new price and volume
    });
    ...
    if (selectedCoin) updateCoinInfo(); // Update coin info with latest details
  }
  ...
  window.selectCoin = function (coinName) {
    ...
    setupChart();
    updateCoinInfo(); // Update coin information when selecting a coin
  };
  ...
  function addNewCoin() {
    const coinNames = [
    "$Pixel", "$DoggyD", "$Funthom", "$Fara", "$:)", "$Tremch", "$OurTime", "$Gecko", "$SmolCat", "$PepePapa", "$SunnyD", "$Ofc"
  ];
    ...
    const newCoin = {
      ...
      volume: 1000000, // Set initial volume to ensure market cap starts at 50,000
      marketCap: 50000 // Set starting market cap at 50,000
    };
    newCoin.marketCap = newCoin.price * newCoin.volume; // Calculate market cap for the new coin
    coins.push(newCoin);
    updateMarket();
  }
